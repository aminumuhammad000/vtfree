...
export class AppGeneratorService {
    ...
    static APPS_BASE_PATH = path.resolve(__dirname, '../../../apps');
    /**
     * Prepares an isolated build environment for a specific app ID and job ID.
     */
    static async prepareBuildDir(appId, jobId) {
     ...
     */
    static async injectConfig(buildDir, options) {
...
`;
     ...
     */
    static async installDependencies(buildDir) {
     ...
     */
    static async runBuild(buildDir, target) {
     ...
     */
    static async cleanup(appId, jobId) {
        const buildDir = path.join(this.APPS_BASE_PATH, appId, 'builds', jobId);
