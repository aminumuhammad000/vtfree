...
import { AdminUser, AuditLog, Transaction, User, ApiKey, FeeRule, RiskRule } from '../models/index.js';
...
export class AdminController {
        ...
        try {
            ...
            if (status === 'active' && oldStatus !== 'active') {
                try {
                    ...
                    await EmailService.sendKYCApproval(user.email, `${user.first_name} ${user.last_name}`, {});
                }
                catch (emailError) {
        ...
        try {
            ...
            if (updates.kyc_status === 'verified' && oldUser.kyc_status !== 'verified') {
                try {
                    ...
                    console.log(`📧 Sending KYC Approval email to ${user.email} `);
                    await EmailService.sendKYCApproval(user.email, `${user.first_name} ${user.last_name}`, {});
                }
                catch (emailError) {
    ...
    static async revokeApiKey(req, res) {
        try {
            ...
            if (!apiKey)
                return ApiResponse.error(res, 'API Key not found', 404);
    ...
    static async updateFee(req, res) {
        try {
            ...
            if (!fee)
                return ApiResponse.error(res, 'Fee rule not found', 404);
    ...
    static async createRiskRule(req, res) {
        try {
    ...
    static async updateRiskRule(req, res) {
        try {
            const rule = await RiskRule.findByIdAndUpdate(req.params.id, req.body, { new: true });
            if (!rule)
                return ApiResponse.error(res, 'Risk rule not found', 404);
    ...
    static async deleteRiskRule(req, res) {
        try {
        ...
        try {
            ...
            if (settings.integrations && settings.integrations.vtstack) {
                ...
                if (vts.isLive !== undefined)
                    await configService.set('VTSTACK_IS_ACTIVE', String(vts.isLive));
            }
