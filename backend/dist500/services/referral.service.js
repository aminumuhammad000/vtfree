...
export class ReferralService {
    ...
    static async processFirstTransactionReferral(userId) {
        try {
            ...
            // Replaced with fallback logic as CreatedApp is deprecated
            // We assume a global referral amount config could exist, for now disabled.
            ...
            await WalletService.credit(user.referred_by, referralAmount, `Referral Bonus: ${user.first_name} ${user.last_name}`, 'referral_bonus');
            // Mark bonus as claimed
            user.referral_bonus_claimed = true;
            await user.save();
            console.log(`Referral bonus of ₦${referralAmount} awarded to referrer`);
            return true;
        }
        catch (error) {
