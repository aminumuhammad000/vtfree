            ...
            switch (type) {
                ...
                case 'plans':
                    ...
                    if (code.toLowerCase() === 'ibdata' && Array.isArray(data)) {
                        const globalPlans = await mongoose.model('AirtimePlan').find({ app_id: null });
                        data = data.map((p) => {
                            const globalPlan = globalPlans.find((gp) => gp.externalPlanId === p.plan_id || gp.code === `IBDATA_${p.plan_id}`);
                            if (globalPlan) {
                                return {
                                    ...p,
