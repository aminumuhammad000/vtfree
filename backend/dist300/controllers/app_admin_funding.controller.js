...
export class AppAdminFundingController {
    static async listAccounts(req, res) {
        try {
            ...
            const allAccounts = [...normalizedVirtual, ...normalizedManual].sort((a, b) => new Date(b.created_at || b.createdAt).getTime() - new Date(a.created_at || a.createdAt).getTime());
     ...
     */
    static async generateVirtualAccount(req, res) {
        try {
            const app_id = req.user?.app_id;
            ...
            if (requestedBank && existingAccounts.find(a => a.metadata?.bankType === requestedBank)) {
     ...
     */
    static async getIBDataBalance(req, res) {
        try {
            const app_id = req.user?.app_id;
            const CreatedApp = (await import('../models/created_app.model.js')).default;
            const app = await CreatedApp.findOne({ app_id });
     ...
     */
    static async getVTPayAccounts(req, res) {
        try {
            const { VTPayService } = await import('../services/vtpay.service.js');
            const CreatedApp = (await import('../models/created_app.model.js')).default;
        ...
        try {
            ...
            const providers = [
                ...
                },
                {
                    ...
                    currency: 'NGN',
                    status: smeplugRes.error ? 'error' : 'ok'
                },
                {
                    code: 'topupmate',
                    name: 'Topupmate',
                    ...
                    currency: 'NGN',
                    status: topupmateRes.error ? 'error' : 'ok'
                },
                {
                    code: 'vtpay',
                    name: 'VTPay',
                    ...
                    currency: 'NGN',
                    status: vtpayRes.error ? 'error' : 'ok'
                }
            ];
