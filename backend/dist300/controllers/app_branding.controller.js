...
export const updateBranding = async (req, res) => {
    try {
        const app_id = req.user.app_id;
        const { app_display_name, app_tagline, colors } = req.body;
        ...
        if (colors) {
            if (colors.primary)
                app.branding.primary_color = colors.primary;
            if (colors.secondary)
                app.branding.secondary_color = colors.secondary;
            if (colors.accent)
                app.branding.accent_color = colors.accent;
            if (colors.background)
                app.branding.background_color = colors.background;
            if (colors.sidebarStart)
                app.branding.sidebar_bg_start = colors.sidebarStart;
            if (colors.sidebarEnd)
                app.branding.sidebar_bg_end = colors.sidebarEnd;
        }
