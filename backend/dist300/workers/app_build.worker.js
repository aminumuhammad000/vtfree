...
const worker = new Worker('app-build-queue', async (job) => {
    ...
    try {
        ...
        await CreatedApp.updateOne({ app_id: appId }, { build_status_full: 'building', build_progress: 10, build_stage: 'Preparing environment' });
        // 2. Handle Cloudinary/External Logo Download
        ...
        await CreatedApp.updateOne({ app_id: appId }, { build_progress: 25, build_stage: 'Configuring template' });
        ...
        await CreatedApp.updateOne({ app_id: appId }, { build_progress: 40, build_stage: 'Syncing to GitHub' });
        ...
        // 6. Run Build
        const artifactPath = await AppGeneratorService.runBuild(buildDir, options.target);
        await job.updateProgress(85);
        await CreatedApp.updateOne({ app_id: appId }, { build_progress: 85, build_stage: 'Uploading to storage' });
        // 7. Upload to Google Drive
        console.log(`[Worker] Uploading artifact to Google Drive`);
        const driveLink = await GoogleDriveService.uploadFile(artifactPath, `${options.app_name}_${jobId}.zip`, 'application/zip');
        // 8. Update DB to completed
        await CreatedApp.updateOne({ app_id: appId }, {
            ...
            download_links: {
                ...
                android: options.target.includes('android') ? driveLink : undefined,
                web: options.target === 'web' ? driveLink : undefined
            },
            last_build_id: jobId
        });
        // 9. Cleanup
