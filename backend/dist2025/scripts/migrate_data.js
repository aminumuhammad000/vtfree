...
async function migrateData() {
    try {
        ...
        const userResult = await User.updateMany({ app_id: { $exists: false } }, { $set: { app_id: appId } });
        console.log(`✅ Migrated ${userResult.modifiedCount} users.`);
        // Migrate Transactions
        const { Transaction } = await import('../models/transaction.model.js');
        const txResult = await Transaction.updateMany({ app_id: { $exists: false } }, { $set: { app_id: appId } });
