...
const updatePriorities = async () => {
    try {
        ...
        const smeResult = await ProviderConfig.updateMany({ code: 'smeplug' }, { $set: { priority: 1, active: true } });
        ...
        const vtstackResult = await ProviderConfig.updateMany({ code: 'vtstack' }, { $set: { priority: 1, active: true } });
        console.log(`Updated ${vtstackResult.modifiedCount} VTStack providers to Priority 1`);
        // 3. Deactivate all other legacy providers
        const legacyResult = await ProviderConfig.updateMany({ code: { $nin: ['smeplug', 'vtstack'] } }, { $set: { active: false, priority: 100 } });
