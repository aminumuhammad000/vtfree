...
export class ReferralService {
     ...
     */
    static async processFirstTransactionReferral(userId: Types.ObjectId | string): Promise<boolean> {
        try {
            ...
            await WalletService.credit(
                user.referred_by,
                appData.referral_settings.amount,
                `Referral Bonus: ${user.first_name} ${user.last_name}`,
                'referral_bonus',
                user.app_id
            );
