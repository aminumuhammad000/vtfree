...
export interface CreatePayrantVASettings {
    documentNumber: string;
    documentType: 'nin' | 'bvn' | 'personal_nin';
...
export class PayrantService {
    private static baseURL = 'https://api-core.payrant.com/';
     ...
     */
    static async createVirtualAccount(data: CreatePayrantVASettings, customApiKey?: string) {
        try {
     ...
     */
    static async validateAccount(bank_code: string, account_number: string, customApiKey?: string) {
        try {
            const client = await this.getClient(customApiKey);
     ...
     */
    static async verifyCheckout(reference: string, customApiKey?: string) {
        try {
            const client = await this.getClient(customApiKey);
