...
export class AppGeneratorService {
    ...
    private static OUTPUT_BASE_PATH = path.resolve(__dirname, '../../../generated_apps');
    ...
    static async generateSourceCode(data: {
        ...
        try {
            ...
            */
            const brandingWithImages = branding as any; // Cast to access potential new props
...
`;
    ...
    static async buildApk(app_id: string, userEmail?: string, onProgress?: (stage: string, progress: number) => Promise<void>): Promise<{ success: boolean; apkPath?: string; message?: string; driveLink?: string }> {
        const appPath = path.join(this.OUTPUT_BASE_PATH, app_id);
        ...
        try {
            ...
            if (onProgress) await onProgress('Installing dependencies...', 10);
            ...
            if (onProgress) await onProgress('Generating native code...', 30);
            ...
            if (onProgress) await onProgress('Compiling APK (this may take a while)...', 50);
            ...
            try {
                ...
                if (onProgress) await onProgress('Running Gradle build...', 60);
            ...
            if (onProgress) await onProgress('Uploading/Finalizing...', 95);
            ...
            if (onProgress) await onProgress('Build Complete!', 100);
    ...
    static async zipSourceCode(app_id: string): Promise<string> {
