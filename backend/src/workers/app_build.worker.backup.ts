...
const worker = new Worker('app-build-queue', async (job: Job) => {
    ...
    const log = (msg: string) => {
        logger.info(`[Worker] [Job:${jobId}] [App:${appId}] ${msg}`);
    };
            ...
            if (releaseId && repoName) {
                ...
                for (const target of targets) {
                    ...
                    try {
                        ...
                        const downloadLink = await githubService.uploadReleaseAsset(repoName, releaseId, artifactPath, fileName);

                        if (target === 'web') links.web = downloadLink;
                        else links.android = downloadLink;
