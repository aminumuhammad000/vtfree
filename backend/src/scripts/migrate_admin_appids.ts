...
import AppAdmin, { IAppAdmin } from '../models/app_admin.model.js';
import CreatedApp from '../models/created_app.model.js';
import VTfreeUser from '../models/vtfree_user.model.js';
import crypto from 'crypto';

/**
 * Migration Script: Convert Admin AppIDs from old format to new structured format
 * New Format: ROLE-OWNER-APP-UNIQUE
 * 
 * Features:
 * - Reads all existing admin records
 * - Maps each record to new ID format using login/owner/app data
 * - Preserves uniqueness (no collisions)
 * - Updates database safely with logging and rollback support
...
interface MigrationLog {
    ...
    email: string;
    role: string;
    status: 'pending' | 'completed' | 'rolled_back' | 'skipped';
    timestamp: Date;
    error?: string;
}
 ...
 */
function getUniqueFromEmail(email: string): string {
 ...
 */
async function generateNewAppId(admin: IAppAdmin): Promise<string> {
    try {
 ...
 */
function isAlreadyMigrated(app_id: string): boolean {
 ...
 */
async function ensureUnique(baseAppId: string, existingIds: Set<string>): Promise<string> {
    let appId = baseAppId;
    let counter = 1;
