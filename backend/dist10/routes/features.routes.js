...
router.patch('/:id', authenticateToken, requireSuperAdmin, async (req, res) => {
    try {
        ...
        const feature = await Feature.findOneAndUpdate({ feature_id: id }, { ...updates, updated_at: new Date() }, { new: true, runValidators: true });
        if (!feature) {
            return res.status(404).json({
                success: false,
                message: 'Feature not found'
...
router.delete('/:id', authenticateToken, requireSuperAdmin, async (req, res) => {
    try {
        const { id } = req.params;
        const feature = await Feature.findOneAndUpdate({ feature_id: id }, { is_active: false, updated_at: new Date() }, { new: true });
        if (!feature) {
            return res.status(404).json({
                success: false,
                message: 'Feature not found'
