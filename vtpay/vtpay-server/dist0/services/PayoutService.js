...
class PayoutService {
    ...
    async calculateFees(amount, isInternal) {
        ...
        if (isInternal) {
            ...
            // VTPay fee (e.g. 2% as per prompt, but we use settings if available)
            // The prompt says "Optional VTPay fee (e.g., 2%)"
            // Let's use 2% if not specified in settings or if we want to follow prompt strictly
            const vtpayFeePercent = 2.0; // Prompt example
            fee = Math.ceil(amount * (vtpayFeePercent / 100));
            // Payrant fee - we'll use a default or fetch from settings
            // Prompt says "Payrant transfer fee is returned in API response"
            // For calculation before API call, we use a placeholder or settings
            payrantFee = payoutSettings.bankSettlementFee;
        }
