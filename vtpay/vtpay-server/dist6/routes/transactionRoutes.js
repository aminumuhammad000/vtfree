...
router.post('/transfer', async (req, res) => {
    try {
        ...
        const pendingTransaction = await services_1.walletService.createPendingTransaction(userId, amountNumber, 0, // Fee will be determined by Zainpay
        'transfer', narration || `Transfer to ${destinationAccountNumber}`, txnRef, {
            destinationAccountNumber,
            destinationBankCode,
        });
        // Lock funds
        await services_1.walletService.lockFunds(userId, amountNumber);
        try {
            // Get user's Zainbox
            const userZainbox = await models_1.Zainbox.findOne({ userId: new mongoose_1.default.Types.ObjectId(userId) });
            if (!userZainbox) {
                await services_1.walletService.unlockFunds(userId, amountNumber);
            ...
            const transferResponse = await services_1.zainpayService.fundTransfer({
                destinationAccountNumber,
                destinationBankCode,
                ...
                callbackUrl: config_1.default.webhookBaseUrl,
            });
            ...
            await services_1.walletService.unlockFunds(userId, amountNumber);
            await services_1.walletService.debitWallet(userId, amountNumber, parseInt(transferData.txnFee || '0', 10), 'transfer', narration || `Transfer to ${destinationAccountNumber}`, txnRef, {
...
router.get('/:txnRef/status', async (req, res) => {
    try {
        ...
        const verifyResponse = await services_1.zainpayService.verifyTransfer(txnRef);
        res.json({
            success: true,
            data: {
