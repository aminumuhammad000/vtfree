...
router.post('/zainboxes/actions/sync', async (req, res) => {
    try {
        ...
        if (response.code === '00' && response.data) {
            ...
            const allZainboxes = await models_1.Zainbox.find({});
            console.log(`Fetching balances for ${allZainboxes.length} Zainboxes...`);
            for (const zBox of allZainboxes) {
                try {
                    const balRes = await ZainpayService_1.zainpayService.getZainboxBalance(zBox.zainboxCode);
...
router.get('/debug/zainpay-logs', async (req, res) => {
    res.json({ success: true, logs: ZainpayService_1.ZainpayService.lastLogs });
});
                ...
                try {
                    ...
                    if (settings && settings.globalSettlement?.status && settings.globalSettlement.settlementAccounts?.length > 0) {
                        ...
                        const settlementPayload = {
                            ...
                            settlementAccountList: settings.globalSettlement.settlementAccounts.map((acc) => ({
                        ...
                        const settlementResponse = await ZainpayService_1.zainpayService.createSettlement(settlementPayload);
 ...
 */
router.post('/sync-zainboxes', async (req, res) => {
        ...
        if (settlementSettings?.settlementAccounts?.length > 0) {
            ...
            (async () => {
                try {
                    ...
                    const cursor = models_1.Zainbox.find({ isActive: true }).cursor();
                    const processingAndPending = [];
                        ...
                        const task = async () => {
                            try {
                                ...
                                await ZainpayService_1.zainpayService.createSettlement({
                                    ...
                                    settlementAccountList: settlementSettings.settlementAccounts.map((acc) => ({
                                        accountNumber: acc.accountNumber,
                                        bankCode: acc.bankCode,
                                        percentage: acc.percentage
                                    })),
 ...
 */
router.get('/settlements/:zainboxCode/schedule', async (req, res) => {
    try {
        const { zainboxCode } = req.params;
        const schedule = await ZainpayService_1.zainpayService.getSettlement(zainboxCode);
        res.json({
            success: true,
 ...
 */
router.post('/settlements/:zainboxCode/schedule', async (req, res) => {
    try {
        const { zainboxCode } = req.params;
        ...
        const totalPercentage = settlementAccountList.reduce((sum, acc) => {
        ...
        const result = await ZainpayService_1.zainpayService.createSettlement(payload);
        res.json({
            success: true,
            data: result,
 ...
 */
router.delete('/settlements/:zainboxCode/schedule', async (req, res) => {
    try {
        const { zainboxCode } = req.params;
        // Get current settlement
        const current = await ZainpayService_1.zainpayService.getSettlement(zainboxCode);
        if (!current.data) {
            res.status(404).json({
                success: false,
 ...
 */
router.post('/settlements/bulk-configure', async (req, res) => {
    try {
