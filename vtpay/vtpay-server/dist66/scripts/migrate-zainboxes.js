    ...
    try {
        ...
        for (const user of users) {
            try {
                ...
                const response = await services_1.zainpayService.createZainbox(payload);
                ...
                if (err.response?.data?.code === '16' || err.message.includes('already exist')) {
                    ...
                    try {
                        const listResponse = await services_1.zainpayService.listZainboxes();
                        if (listResponse.code === '00' && listResponse.data) {
                            ...
                            const match = allZainboxes.find((z) => z.emailNotification === user.email ||
...
migrateZainboxes();
//# sourceMappingURL=migrate-zainboxes.js.map